...
class AutomationGatewayConfig:
    ...
    @classmethod
    def available_templates(cls, templates_dir: Optional[Path] = None) -> set[str]:
        directory = templates_dir or AUTOMATION_GATEWAY_TEMPLATES_DIR
        templates: set[str] = set()
        if directory.exists():
            for path in directory.glob(f"*{cls.TEMPLATE_SUFFIX}"):
